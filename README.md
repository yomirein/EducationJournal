...
## Почта: подтверждение и восстановление пароля

- **Подтверждение.** После регистрации платформа отправляет письмо со ссылкой `{FRONTEND_URL}/auth/verify.html?token=...`. **Войти можно только после подтверждения почты** — до этого вход отвечает 403, а страница входа предлагает отправить письмо ещё раз. Администратор может подтвердить почту вручную (раздел «Пользователи»).
- **Восстановление пароля.** На `auth/reset.html` пользователь вводит логин или почту и получает ссылку `{FRONTEND_URL}/auth/reset.html?token=...` (`POST /auth/forgot-password`, `POST /auth/reset-password`). Ссылка действует `PASSWORD_RESET_TOKEN_EXPIRE` минут (по умолчанию 60) и срабатывает один раз. Ответ на запрос не раскрывает, существует ли аккаунт.

Письма уходят по **SMTP** в фоне: запрос не ждёт письма и не падает, если почта не настроена или сервер недоступен — причина пишется в лог (`pixelstart.email`). Аккаунты, созданные до введения обязательного подтверждения, миграция `0004` отмечает подтверждёнными.
...
- **Куратор:** видит очередь работ своих потоков, оценивает их и отправляет объявления. На дашборде блок «Кому нужна помощь» показывает учеников, которые отстают от графика потока на 20% и больше или не сдают работы 7 дней.
- **Админ:** создаёт курсы, уроки, задания и потоки, управляет ролями и оплатой пользователей, имеет доступ ко всем потокам.

### Прогресс и рейтинг

Правила считаются в одном месте — `backend/app/progress.py` — и показываются ученику в журнале и на странице рейтинга:

- шаг зачтён при оценке от 50; задача с тестами — только когда пройдены все тесты (100);
- зачтённый шаг даёт баллы, равные оценке; оценка куратора засчитывается так же, как автоматическая проверка;
- работа на проверке баллов не даёт, пока куратор её не оценит;
- рейтинг курса — сумма баллов за его шаги, место — среди учеников того же потока.

В журнале у каждого курса есть разбивка по шагам (кто проверял, статус, баллы), место в потоке, сравнение с графиком потока и кнопка следующего шага. Ожидаемый прогресс — доля прошедшего времени потока (между датами начала и окончания).
