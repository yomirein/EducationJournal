...
document.querySelectorAll('button[data-load]').forEach(button => {
  button.addEventListener('click', () => run(async () => {
    ...
    if (output) {
      output.textContent = JSON.stringify(await api.get(button.dataset.load), null, 2);
    }
  }, 'Данные обновлены.'));
});
    ...
    loadCourses.innerHTML = courses.map(course => {
      ...
      return `<article class="card course-card" data-course-type="${course.type}">
        <div class="course-art ${colorClass}">
          <span class="art-label">${course.type.toUpperCase()}</span>
          <div class="art-code">API COURSE #${course.id}</div>
        </div>
        <div class="course-body">
          ...
          <h3>${course.title}</h3>
          <p>${course.description || 'Описание курса появится после публикации администратором.'}</p>
          <a class="button button-dark" href="course/index.html?course=${course.id}">Открыть курс <span>↗</span></a>
        </div>
      </article>`;
    }).join('');
  }, 'Каталог обновлён.');
