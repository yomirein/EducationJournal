...
from sqlalchemy import Boolean, CheckConstraint, DateTime, Enum as SAEnum, Float, ForeignKey, Index, Integer, String, Table, Text, UniqueConstraint, Column
...
course_modules = Table("course_modules", Base.metadata,
...
class User(Base):
    ...
    role: Mapped[UserRole] = mapped_column(SAEnum(UserRole, name="user_roles"), default=UserRole.student, index=True)
...
class Course(Base):
    ...
    modules: Mapped[list["Module"]] = relationship(secondary=course_modules, back_populates="courses")
...
class Module(Base):
    ...
    description: Mapped[str] = mapped_column(Text, default="")
    courses: Mapped[list[Course]] = relationship(secondary=course_modules, back_populates="modules")
...
class Stream(Base):
    ...
    curator_id: Mapped[int] = mapped_column(ForeignKey("users.id", ondelete="RESTRICT"), index=True)
    name: Mapped[str] = mapped_column(String(200))
...
class StreamParticipant(Base):
    ...
    __table_args__ = (UniqueConstraint("user_id", "stream_id"), CheckConstraint("status IN ('pending','accepted','rejected')"))
    id: Mapped[int] = mapped_column(primary_key=True)
...
class Submission(Base):
    ...
    __table_args__ = (UniqueConstraint("task_id", "user_id"), CheckConstraint("grade >= -1 AND grade <= 100", name="ck_submission_grade"))
    id: Mapped[int] = mapped_column(primary_key=True)
...
class StreamBroadcast(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    stream_id: Mapped[int] = mapped_column(ForeignKey("streams.id", ondelete="CASCADE"), index=True)
    broadcast_id: Mapped[int] = mapped_column(ForeignKey("broadcasts.id", ondelete="CASCADE"), unique=True)
