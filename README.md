...
│   └── requirements.txt
├── front/
│   ├── assets/                      # api.js (API-клиент), js/ (логика), css/ (стили компонентов и страниц)
...
CSS в `front/assets/css/` подключается явно в HTML, без сборки:

- `core/base.css` — базовые правила, переменные и палитры светлой/тёмной темы.
- `shared/ui.css` и `shared/layout.css` — общие элементы интерфейса и каркас страниц; подключаются после базы на всех страницах.
- Остальные файлы `shared/` — повторно используемые компоненты: карточки курсов, участники, содержимое заданий, уведомления и профиль.
- `student/`, `curator/`, `admin/`, `public/` — стили конкретных страниц; подключаются после нужных общих компонентов.

Мобильные правила и тёмные варианты находятся рядом со стилями своего компонента. Порядок подключений влияет на каскад: при его изменении нужно проверять обе темы и узкий экран. После правок ассетов выполните `python scripts/bump_assets.py`, чтобы обновить версии ссылок во всех HTML. CSS встроенных симуляторов остаётся в `front/simulators/`.
...
### Перезапуск с актуальными изменениями

Контейнер `api` содержит **копию** кода на момент сборки: после `git pull` или правок в `backend/` и `front/` его нужно пересобрать, иначе сайт покажет старую версию. Команда пересобирает образ, применяет новые миграции и перезапускает только то, что изменилось; данные базы сохраняются:

```bash
docker compose up --build -d
```

`docker compose restart` и `docker compose up -d` без `--build` новых файлов **не подхватывают**. Если после пересборки видна старая версия:

```bash
# Сборка с нуля, без кеша Docker
docker compose build --no-cache api && docker compose up -d
# Логи API (миграции, ошибки, отправка писем)
docker compose logs -f api
```

В браузере обновите страницу без кеша: Cmd+Shift+R (macOS) или Ctrl+F5 (Windows).
...
- **Смена почты и пароля.** В «Настройках» ученик меняет имя, фамилию, почту и пароль. Для смены почты и пароля нужен текущий пароль. Новая почта (`pending_email`) начинает действовать только после перехода по ссылке `{FRONTEND_URL}/auth/verify.html?change=1&token=...`, отправленной на неё (`POST /auth/confirm-email-change`); до этого вход и письма идут на прежний адрес.
...
- **Уведомления:** объявления кураторов. Колокольчик в шапке показывает три последних и счётчик непрочитанных, кнопка «Все уведомления» ведёт на страницу уведомлений ученика (новые сверху) или в раздел объявлений куратора. О новом непрочитанном уведомлении один раз сообщает всплывающее сообщение. Отметки «прочитано» хранятся в браузере.
